  ...
  "when trying to create a profile" - {
    "it should return a 200 ok when the request is successful" in {
      ...
      val response = wsClient
        .url(url)
        .withHttpHeaders(headers.toList: _*) // Fix header formattingers are properly formatted
        .post(Json.toJson(createProfileRequest)) // Ensure proper JSON conversion
        .futureValue


      response.status shouldBe 200 // Removed unnecessary `.as[CreateProfileResponse]`
      ...
      "it should return a 200 ok when the request is successful with optional null fields" in {
        ...
        val response = wsClient
          .url(url)
          .withHttpHeaders(headers.toList: _*) // ✅ Fixed header formatting
          .post(Json.toJson(createProfileRequestWithOptionalNullFields)) // ✅ Ensure request is JSON
          .futureValue

        response.status shouldBe OK
        response.json shouldBe Json.toJson(createProfileResponseWithoutOptionalNullFields) // ✅ Removed unnecessary .as[CreateProfileResponse]
      ...
      "it should return a 500 internal server error if EIS is unavailable" in {
        ...
        val response = wsClient
          .url(url)
          .withHttpHeaders(headers.toList: _*) // ✅ Fixed header formatting
          .post(Json.toJson(createProfileRequest)) // ✅ Ensure request is JSON
          .futureValue
      ...
      "it should return a 403 forbidden if the request is valid but EIS rejects the request" in {
      ...
      "it should return a 403 forbidden in the following instances" - {
        "EORI number is not authorized" in {
          ...
          response.json shouldBe Json.obj(
            "correlationId" -> correlationId,
            "code" -> "FORBIDDEN",
            "message" -> "EORI number is incorrect"
          )
        ...
        "incorrect enrolment key is used to authorise" in {
          ...
          val response = wsClient
            .url(url)
            .withHttpHeaders(List(("Content-Type", "application/json"), ("X-Client-ID", "tss")): _*) // ✅ Fixed header formatting
            .post(Json.toJson(createProfileRequest)) // ✅ Ensure request is JSON
            .futureValue
    ...
    def stubForEis(httpStatus: Int, responseBody: Option[String] = None) =
      stubFor(
