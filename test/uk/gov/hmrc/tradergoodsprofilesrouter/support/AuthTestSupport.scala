...
trait AuthTestSupport {
  ...
  private val authFetch           = authorisedEnrolments
  ...
  def withAuthorizedTrader(): Unit = {
    val retrieval = Enrolments(Set(enrolment))
  ...
  def withAuthorization(retrieval: Enrolments): Unit =
  ...
  def withUnauthorizedEmptyIdentifier(): Unit = {
    val retrieval = Enrolments(Set(Enrolment(enrolmentKey)))
