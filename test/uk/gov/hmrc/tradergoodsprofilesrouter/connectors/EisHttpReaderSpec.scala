...
class EisHttpReaderSpec extends PlaySpec with GetRecordsDataSupport with EitherValues {
  ...
  "remove record responseHandler" should {
    "remove a record item" in {
      ...
      val result      = RemoveRecordHttpReader(correlationId, successErrorHandler).read("PUT", "any-url", eisResponse)
    ...
    "handle error response" in {
      ...
      RemoveRecordHttpReader[Int](correlationId, errorHandlerSpy.handleErrorResponse)
        .read("GET", "any-url", eisResponse)
  ...
  private def spyOnHandlerErrorFn = {
    ...
    doReturn(Ok("Success")).when(errorHandlerSpy).handleErrorResponse(any, any)
    errorHandlerSpy
  }
